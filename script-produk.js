const FALLBACK_PRODUCTS = [
  ...
  { id:"demolish-200", name:"Demolish 18 EC 200ml", cat:"insektisida", size:"200 ml", img:"images/produk/demolish-200.webp", desc:"Insektisida racun kontak & lambung untuk berbagai hama sayuran dan perkebunan.", activeIngredient:"Abamektin 18 g/L", target:"Hama pada cabai, bawang merah, kubis, padi, kentang, dan kelapa sawit", long:"Insektisida racun kontak dan lambung berwarna coklat kehitaman, berbentuk pekatan yang dapat diemulsikan. Digunakan untuk mengendalikan hama pada tanaman cabai, bawang merah, kubis, padi, kentang, dan kelapa sawit." },
  { id:"glufo-1l", name:"Glufo 150 SL 1 Liter", cat:"herbisida", size:"1 liter", img:"images/produk/glufo-1l.webp", desc:"Herbisida sistemik purna tumbuh non-selektif untuk gulma di perkebunan sawit & kakao.", activeIngredient:"Amonium Glufosinat 150 g/L", target:"Gulma berdaun lebar dan berdaun sempit pada kelapa sawit (TBM) dan kakao (TBM)", long:"Herbisida sistemik purna tumbuh non-selektif berbentuk pekatan berwarna biru kehijauan yang dapat larut dalam air. Digunakan untuk mengendalikan gulma berdaun lebar maupun berdaun sempit pada areal perkebunan kelapa sawit dan kakao yang belum menghasilkan (TBM)." },
  ...
  { id:"starlon-100", name:"Starlon 665 EC 100ml", cat:"herbisida", size:"100 ml", img:"images/produk/starlon-100.webp", desc:"Herbisida sistemik spesialis gulma berkayu dan tunggul pohon.", activeIngredient:"Triklopir Butoksi Etil Ester 665 g/L", target:"Semak belukar, gulma berkayu, dan tunggul pohon pada kelapa sawit dan padi", long:"Herbisida sistemik purna tumbuh produksi Nufarm, sangat efektif mengendalikan semak belukar, tunggul kayu, dan gulma berkayu. Bekerja meresap ke dalam jaringan kayu hingga ke akar sehingga tunggul pohon tidak tumbuh kembali. Dapat dicampur dengan herbisida glifosat atau parakuat untuk memperluas spektrum pengendalian, dan dapat diaplikasikan dengan cara semprot maupun oles pada tunggul." },
  ...
  { id:"kresnaup-5l", name:"Kresna Up 520 SL 5 Liter", cat:"herbisida", size:"5 liter", img:"images/produk/kresnaup-combo.webp", desc:"Herbisida sistemik purna tumbuh untuk gulma berdaun lebar dan sempit.", activeIngredient:"IPA Glifosat 520 g/L", target:"Gulma berdaun lebar dan sempit pada kelapa sawit belum menghasilkan (TBM)", long:"Herbisida sistemik purna tumbuh produksi Sari Kresna Kimia, berbentuk larutan dalam air berwarna cokelat keemasan. Diserap melalui daun dan bekerja sistemik hingga ke akar gulma. Cocok digunakan pada perkebunan kelapa sawit yang belum menghasilkan (TBM). Kemasan 5 liter untuk kebutuhan penyemprotan area lebih luas." },
  { id:"okboss-5l", name:"Ok Boss 540 SL 5 Liter", cat:"herbisida", size:"5 liter", img:"images/produk/okboss-combo.webp", desc:"Herbisida sistemik purna tumbuh konsentrasi tinggi untuk gulma berdaun lebar dan sempit.", activeIngredient:"Isopropil Amina Glifosat 540 g/L", target:"Gulma berdaun lebar dan berdaun sempit pada lahan perkebunan", long:"Herbisida sistemik purna tumbuh dengan konsentrasi glifosat tinggi, berbentuk larutan dalam air. Bekerja diserap melalui daun dan disebarkan hingga ke akar gulma, efektif untuk mengendalikan gulma berdaun lebar maupun sempit yang mengganggu lahan perkebunan. Kemasan 5 liter untuk kebutuhan penyemprotan area lebih luas." },
...
async function loadProducts(){
  ...
  try{
    ...
    products = rows.filter(r => !r.is_hidden).map(mapSupabaseRow);
  } catch(err){
...
// ===== Carousel foto detail produk (maks 3 foto) =====
...
function renderCarousel(images){
  ...
  carouselIndex = 0;
  detailCarouselTrack.innerHTML = images.map((src, i) => `<img src="${src}" alt="Foto produk ${i+1}">`).join('');
  carouselDots.innerHTML = images.length > 1
    ? images.map((_, i) => `<button class="dot${i===0?' active':''}" data-idx="${i}" aria-label="Foto ${i+1}"></button>`).join('')
    : '';
  const showArrows = images.length > 1;
  if(carouselPrev) carouselPrev.classList.toggle('show', showArrows);
  if(carouselNext) carouselNext.classList.toggle('show', showArrows);
  updateCarouselPosition();

  detailCarouselTrack.querySelectorAll('img').forEach(img => {
    img.addEventListener('click', openDetailLightbox);
  });
...
function updateCarouselPosition(){
  detailCarouselTrack.style.transform = `translateX(-${carouselIndex * 100}%)`;
  carouselDots.querySelectorAll('.dot').forEach((dot, i) => dot.classList.toggle('active', i === carouselIndex));
}
...
if(detailCarousel){
  detailCarousel.addEventListener('touchstart', (e) => { carouselTouchStartX = e.touches[0].clientX; }, { passive: true });
...
function openDetailLightbox(){
  ...
  detailLightboxImg.src = currentSrc;
...
function openDetail(id, opts = {}){
  ...
  const images = [p.img, p.img2, p.img3].filter(Boolean);
  if(images.length > 0){
    renderCarousel(images);
    if(detailCarousel) detailCarousel.style.display = '';
    detailNoImg.style.display = 'none';
  } else {
    const catMeta = CATEGORIES.find(c => c.id === p.cat);
    if(detailCarousel) detailCarousel.style.display = 'none';
